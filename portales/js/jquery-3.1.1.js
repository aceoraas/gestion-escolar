/ *! * jQuery JavaScript Library v3 .3 .1 * https: //jquery.com/
    * * Incluye Sizzle.js * https: //sizzlejs.com/
    * * Copyright JS Foundation y otros colaboradores * Publicado bajo la licencia de MIT * https: //jquery.org/license
    * * Fecha: 2018 - 01 - 20 T17: 24 Z * / (función(global, fábrica) {
        "usar estricto";
        if (typeof module === "object" && typeof module.exports === "object") {
            ...
            module.exports = global.document ? fábrica(global, verdadero) : función(w) {
                if (!w.document) {
        ...
        // Pase esto si la ventana aún no está definida
            ...
            function toType(obj) {
                ...
                return typeof obj === "objeto" || typeof obj === "función" ? class2type[toString.call(obj)] || "objeto" : typeof obj;
            }
            / * símbolo global * /
            ...
            jQuery.extend({
                ...
                expando: "jQuery" + (versión + Math.random()).replace(/ \ D /
                    g, ""),
                ...
                recortar: función(texto) {
                    devolver texto == nulo ? "" : (texto + "").replace(rtrim, "");
                },
                ...
                makeArray: function(arr, results) {
                    ...
                    if (arr! = null) {
                        if (isArrayLike(Object(arr))) {
                            jQuery.merge(ret, typeof arr === "cadena" ? [arr] : arr);
                        } else {
                            push.call(ret, arr);
                        }
                    }
            ...
            jQuery.each("Boolean Number String Function Array Date RegExp Object Error Symbol".split(""), función(i, nombre) {
            ...
            function isArrayLike(obj) {
                ...
                tipo de retorno === "matriz" || longitud === 0 || typeof length === "number" && length > 0 && (length - 1) en obj;
            }
            var Sizzle = / *! * Sizzle CSS Selector Engine v2 .3 .3 * https: //sizzlejs.com/
                * * Copyright jQuery Foundation y otros colaboradores * Publicado bajo la licencia de MIT * http: //jquery.org/license
                * * Fecha: 2016 - 08 - 08 * / (función(ventana) {
                    var i,
                        apoyo,
                        Expr,
                        getText,
                        ...
                        "* (?: '((?: \\\\ | | [^ \\\\']) *) '| \" ((?: \\\\ | | [^ \\\\\ "]) * )\
        "| (" + identificador + ")) |)" + espacio en blanco + "* \\]", pseudos = ":(" + identificador + ") (?: \\ ((" +
...
"((?: \\\. | [^ \\\\ () [\\]] |" + attributes + ") *) |" +
    ...
    matchExpr = {
        ...
        "CHILD": nuevo RegExp("^ :( solo | first | last | nth | nth-last) - (child | of-type) (?: \\ (" + whitespace + "* (incluso | impar | (([+ -] |) (\\ d *) n |)" + espacio en blanco + "* (?: ([+ -] |)" + espacio en blanco + "* (\\ d +) |)) + espacio en blanco +" * \\) | )
","
i ",
"bool": nuevo RegExp("^ (?:" + booleans + ") $", "i"),
    ...
    "needsContext": nuevo RegExp("^" + espacio en blanco + "* [> + ~] |: (incluso | impar | eq | gt | lt | nth | primero | último) (?: \\ (" + espacio en blanco + "* ((?: - \\ d)? \\ d *)" + espacio en blanco + "* \\) |) (? = [^ -] | $)", "i")
},
rinputs = / ^ (?: input | select | textarea | botón) $ /
i,
rheader = / ^ h \ d $ /
i,
rnative = / ^ [^ {] + \ {\ s * \ [native \ w /,
...
rquickExpr = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/,
    ...
    funescape = function(_, escaped, escapadoWhitespace) {
        ...
        ¡
        vuelve alto! == alto || escapadoWhitespace ? escapado : alto < 0 ?
            // punto de código BMP
...
fcssescape = function(ch, asCodePoint) {
    ...
    disabledAncestor = addCombinator(función(elem) {
        return elem.disabled === true && ("forma"
            en elem || "etiqueta"
            en elem);
    }, {
        dir: "parentNode",
...
function Sizzle(selector, contexto, resultados, semilla) {
    ...
    if (typeof selector! == "cadena" || !selector || nodeType! == 1 && nodeType! == 9 && nodeType! == 11) {
    ...
    if (!seed) {
        ...
        if (documentIsHTML) {
            ...
            if (nodeType! == 11 && (match = rquickExpr.exec(selector))) {
                ...
                if ((m = match[1])) {
                    ...
                    if (nodeType === 9) {
                        ...
                        // TODO: identificar versiones
                        // getElementById puede hacer coincidir elementos por nombre en lugar de ID
                        if (newContext && (elem = newContext.getElementById(m)) && contiene(contexto, elem) && elem.id === m) {
                            results.push(elem);
                            resultados de devolución;
                        }
                    }
            ...
            if (newSelector) {
                tratar {
                    push.apply(resultados, newContext.querySelectorAll(newSelector));
                    resultados de devolución;
                } catch (qsaError) {}
                finalmente {
                    if (nid === expando) {
...
/ ** * Crear cachés clave - valor de tamaño limitado * @returns {
    function(string, object)
}
Devuelve los datos del objeto después de almacenarlo en sí mismo con * nombre de propiedad la cadena(con sufijo de espacio) y(si la caché es más grande que Expr.cacheLength) * borrar la entrada más antigua * /
...
/ ** * Marque una función para uso especial por Sizzle * @param {
    Function
}
fn La función para marcar * /

function markFunction(fn) {
    fn[expando] = verdadero;
    return fn;
}
/ ** * Pruebas de soporte usando un elemento * @param {
    Función
}
fn Pasó el elemento creado y devuelve un resultado booleano * /
función assert(fn) {
...
/ ** * Agrega el mismo controlador para todos los atributos especificados * @param {
    String
}
attrs Lista de atributos separados por tuberías * @param {
    Function
}
handler El método que se aplicará * /
...
/ ** * Verifica el orden de los documentos de dos hermanos * @param {
    Element
}
a * @param {
    Element
...
Devuelve menos de 0 si a es anterior a b, es mayor que 0 si a sigue a b * /
...
/ ** * Devuelve una función para usar en pseudos para los tipos de entrada * @param {
    String
}
tipo * /
...
/ ** * Devuelve una función para usar en pseudos para botones * @param {
    String
}
tipo * /
...
/ ** * Devuelve una función para usar en pseudos para: habilitado / : deshabilitado * @param {
    booleano
}
deshabilitado verdadero para: deshabilitado;
falso para: habilitado * /
...
/ ** * Devuelve una función para usar en pseudos para posiciones * @param {
    Function
}
fn * /
...
/ ** * Comprueba la validez de un nodo como contexto Sizzle * @param {
    Element | Object =
}
context * @return {
    Element | Object | Boolean
}
El nodo de entrada si es aceptable, de lo contrario un valor falso * /
...
/ ** * Detecta nodos XML * @param {
    Element | Object
}
elem Un elemento o documento * @returns {
    Boolean
}
True iff elem es un nodo XML no HTML * /
...
/ ** * Establece las variables relacionadas con el documento una vez sobre la base del documento actual * @param {
    Element | Object
}[doc] Un elemento u objeto de documento para usar para configurar el documento * @returns {
    Object
}
...
setDocument = Sizzle.setDocument = function(node) {
    ...
    if (preferredDoc! == documento && (subWindow = document.defaultView) && subWindow.top! == subWindow) {
        // Soporte: IE 11, Edge
    ...
    // Tag
    Expr.find["TAG"] = support.getElementsByTagName ? función(etiqueta, contexto) {
    ...
    if ((support.qsa = rnative.test(document.querySelectorAll))) {
        ...
        assert(function(el) {
            ...
            docElem.appendChild(el).innerHTML = "<a id='" + expando + "'> </a> " + "<select id = '" + expando + "- \ r \\' msallowcapture = ''>" + "<opción seleccionada = ''> </ option> </ select>";
    ...
    contains = hasCompare || rnative.test(docElem.contains) ? función(a, b) {
        var adown = a.nodeType === 9 ? a.documentElement : a,
            bup = b && b.parentNode;
        devolver a === bup || !!(bup && bup.nodeType === 1 && (adown.contains ? adown.contains(bup) : a.compareDocumentPosition && a.compareDocumentPosition(bup) & 16));
    } : función(a, b) {
        si(b) {
    ...
    sortOrder = hasCompare ? función(a, b) {
        ...
        compare = (a.ownerDocument || a) === (b.ownerDocument || b) ? a.compareDocumentPosition(b) :
            ...
            1;
        // nodos desconectados
        if (comparar y 1 || (!support.sortDetached && b.compareDocumentPosition(a) === compare)) {
        ...
        if (!aup || !bup) {
            devolver un === documento ? -1 : b === documento ? 1 : aup ? -1 : bup ? 1 : sortInput ? (indexOf(sortInput, a) - indexOf(sortInput, b)) : 0;
...
Sizzle.attr = function(elem, name) {
    ...
    var fn = Expr.attrHandle[name.toLowerCase()],
        ...
        val = fn && hasOwn.call(Expr.attrHandle, name.toLowerCase()) ? fn(elem, name, !documentIsHTML) : indefinido;
    return val! == undefined ? Val : support.attributes || !documentIsHTML ? elem.getAttribute(nombre) : (val = elem.getAttributeNode(name)) && val.specified ? val.value : nulo;
};
...
/ ** * Clasificación de documentos y eliminación de duplicados * @param {
    ArrayLike
}
resultados * /
...
/ ** * Función de utilidad para recuperar el valor de texto de una matriz de nodos DOM * @param {
    Array | Element
}
elem * /
